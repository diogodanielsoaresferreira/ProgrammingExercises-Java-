...
public class Ementa2 {
	...
	private static Alimento[] al = new Alimento[0];
	...
	public static void addCarne(){
		...
		addArrayIng(new Carne(Carne.VariedadeCarne.getVar(n),pro,cal,pes));
	}
	...
	public static void addPeixe(){
		...
		addArrayIng(new Peixe(Peixe.TipoPeixe.getTipo(n),pro,cal,pes));
	}
	...
	public static void addCereal(){
		...
		addArrayIng(new Cereal(cer,pro,cal,pes));
	}
	...
	public static void addLegume(){
		...
		addArrayIng(new Legume(leg,pro,cal,pes));
	}
	...
	public static void criaPrato(){
		...
		if (n==1){
			addArrayPrato(new Prato(name));
		}
		else if(n==2){
			out.print("\nEscreva o número máximo de calorias no prato: ");
			int cal = sc.nextInt();
			sc.nextLine();
			addArrayPrato(new PratoDieta(name, cal));
		}
		else{
			addArrayPrato(new PratoVegetariano(name));
		}
	}
	...
	public static void apagaPrato(){
		...
		do{
			...
			n=sc.nextInt();
			sc.nextLine();
		}while(n<1 || n>pr.length);
		n--;
		Prato[] prc = new Prato[pr.length-1];
	...
	public static void addPrato(){
		...
		k--;
		e.addPrato(pr[n], Ementa.DiaSemana.enumDay(k));
	...
	public static void carregarEmenta(){
		...
		try{
			...
			for(int j=0;j<a.length;j+=5){
				j++;
				if(a[j].equals("Carne")){
					al[i++]= new Carne(Carne.VariedadeCarne.getVar(Integer.parseInt(a[j+4])) , Double.parseDouble(a[j+1]), Double.parseDouble(a[j+2]),Double.parseDouble(a[j+3]));
				}
				else if(a[j].equals("Peixe")){
					al[i++]= new Peixe(Peixe.TipoPeixe.getTipo(Integer.parseInt(a[j+4])) , Double.parseDouble(a[j+1]), Double.parseDouble(a[j+2]),Double.parseDouble(a[j+3]));
				}
				else if(a[j].equals("Legume")){
					al[i++]= new Legume(a[j+4] , Double.parseDouble(a[j+1]), Double.parseDouble(a[j+2]),Double.parseDouble(a[j+3]));
				}
				else{
					al[i++]= new Cereal(a[j+4] , Double.parseDouble(a[j+1]), Double.parseDouble(a[j+2]),Double.parseDouble(a[j+3]));
				}
			...
			while(b<pdiv.length){
				if(pdiv[b].equals("PratoNormal")){
					b++;
					pr[k++] = new Prato(pdiv[b++]);
					while(b<pdiv.length && pdiv[b].charAt(0)!='p'){
						b++;
						if(pdiv[b].equals("Carne")){
							pr[k-1].addAl(new Carne(Carne.VariedadeCarne.getVar(Integer.parseInt(pdiv[b+4])) , Double.parseDouble(pdiv[b+1]), Double.parseDouble(pdiv[b+2]),Double.parseDouble(pdiv[b+3])));
						}
						else if(pdiv[b].equals("Peixe")){
							pr[k-1].addAl(new Peixe(Peixe.TipoPeixe.getTipo(Integer.parseInt(pdiv[b+4])) , Double.parseDouble(pdiv[b+1]), Double.parseDouble(pdiv[b+2]),Double.parseDouble(pdiv[b+3])));
						}
						else if(pdiv[b].equals("Legume")){
							pr[k-1].addAl(new Legume(pdiv[b+4] , Double.parseDouble(pdiv[b+1]), Double.parseDouble(pdiv[b+2]),Double.parseDouble(pdiv[b+3])));
						}
						else{
							pr[k-1].addAl(new Cereal(pdiv[b+4] , Double.parseDouble(pdiv[b+1]), Double.parseDouble(pdiv[b+2]),Double.parseDouble(pdiv[b+3])));
						}
						b+=5;
					}
					b++;
					...
					b++;
					pr[k++] = new PratoVegetariano(pdiv[b++]);
					while(b<pdiv.length && pdiv[b].charAt(0)!='p'){
						b++;
						if(pdiv[b].equals("Carne")){
							pr[k-1].addAl(new Carne(Carne.VariedadeCarne.getVar(Integer.parseInt(pdiv[b+4])) , Double.parseDouble(pdiv[b+1]), Double.parseDouble(pdiv[b+2]),Double.parseDouble(pdiv[b+3])));
					...
					b++;
					pr[k++] = new PratoDieta(pdiv[b+1] ,Double.parseDouble(pdiv[b++]));
					b++;
					while(b<pdiv.length && pdiv[b].charAt(0)!='p'){
						b++;
						if(pdiv[b].equals("Carne")){
	...
	private static void addArrayIng(Alimento a){
		Alimento []c = new Alimento[al.length+1];
		for(int i=0;i<al.length;i++){
			c[i]=al[i];
		}
		c[al.length]=a;
	...
	private static void addArrayPrato(Prato p){
