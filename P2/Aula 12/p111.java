...
public class p111
{
   ...
   {
      BinaryAssociativeArray<String> aa = new BinaryAssociativeArray<String>(100);
      int i = 0;
      while(i < args.length)
      {
         if (args[i].equals("set")) // set key value
         {
            ...
            aa.set(Integer.parseInt(args[i+1]), args[i+2]);
            i+=2;
         }
         else if (args[i].equals("get")) // get key
         {
            ...
            if (!aa.exists(Integer.parseInt(args[i+1])))
            {
               err.println("ERROR: aa["+args[i+1]+"] does not exist");
               exit(2);
            }
            out.println("aa["+args[i+1]+"] = "+aa.get(Integer.parseInt(args[i+1])));
            i+=1;
         }
         else if (args[i].equals("delete")) // delete key
         {
            ...
            aa.delete(Integer.parseInt(args[i+1]));
            i+=1;
         }
