...
public class ContainerStack {
	...
	public int searchRec(String type){
		return rSearch(type, size-1);
	}
	
	private int rSearch(String type, int a){
		if(a<0) return -1;
		if(array[a].contains(type)) return -a-1+size;
		else return rSearch(type, a-1);
	}
	
	private void sort (Container[] a, int start, int end){
		if(end-start>1){
